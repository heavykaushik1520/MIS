...
//    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    ...
    // ✅ Update Invoice (e.g., change email or balance)
    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    public ResponseEntity<InvoiceResponseDto> updateInvoice(@PathVariable int id, @RequestBody InvoiceRequestDto dto) {
    ...
    public ResponseEntity<byte[]> downloadInvoicePdf(@PathVariable int id) {
        ByteArrayInputStream pdfStream = invoiceService.generatePdfForInvoice(id);
