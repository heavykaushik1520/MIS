...
function BrandManagement() {
  ...
  const [chainId, setChainId] = useState("");
  const [message, setMessage] = useState("");
  const [isActive, setIsActive] = useState(true); // or false
  ...
  const fetchAllBrands = async () => {
    try {
      ...
      if (response.ok) {
        const brandArray = Array.isArray(data) ? data : data.data;
  ...
  return (
    <div className="p-6 bg-gray-900 text-white shadow-xl rounded-lg w-full max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-purple-400 mb-6">Brand Management</h2>
      ...
      <div className="mb-4">
        <input
          ...
          value={brandId}
          onChange={(e) => setBrandId(e.target.value)}
          className="input input-bordered w-full bg-gray-800 text-white border-gray-600 rounded-lg p-2"
        />
        <button
          onClick={fetchBrandById}
          className="btn bg-blue-600 hover:bg-blue-700 text-white font-semibold w-full mt-2 rounded-lg py-2"
        >
          Get Brand Details
        </button>
      </div>
      ...
      {brandDetails && (
        <div className="p-4 bg-gray-800 text-white rounded-lg shadow-md mt-4">
          <h3 className="text-xl font-semibold text-yellow-400">Brand Info</h3>
          <p><strong>Name:</strong> {brandDetails.brandName}</p>
          <p><strong>Active:</strong> {brandDetails.isActive ? "Yes" : "No"}</p>
          ...
          <div className="flex mt-4 space-x-2">
            <button
              onClick={() => navigate(`/brands/update/${brandDetails.brandId}`)}
              className="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-lg w-1/2"
            >
              Update Brand
            </button>
            <button
              onClick={() => deleteBrand(brandDetails.brandId)}
              className="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg w-1/2"
            >
              Delete Brand
            </button>
          </div>
      ...
      <h3 className="text-lg font-semibold text-blue-400 mt-6">All Brands</h3>
      ...
      <ul className="list-none mt-2">
        {Array.isArray(brands) && brands.map((b) => (
          ...
          >
            <h3 className="text-lg font-semibold text-yellow-400">Brand Info</h3>
            <p><strong>Name:</strong> {b.brandName}</p>
            <p><strong>Active:</strong> {b.isActive ? "Yes" : "No"}</p>

            {b.chain && (
              <>
                <h3 className="text-md font-semibold text-green-400 mt-2">Chain Info</h3>
                <p><strong>Company:</strong> {b.chain.companyName}</p>
                <p><strong>GSTN:</strong> {b.chain.gstnNo}</p>
              </>
            )}
            ...
            <div className="flex mt-3 space-x-2">
              <button
                onClick={() => navigate(`/brands/update/${b.brandId}`)}
                className="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-1 px-3 rounded-lg"
              >
                Update
              </button>
              <button
